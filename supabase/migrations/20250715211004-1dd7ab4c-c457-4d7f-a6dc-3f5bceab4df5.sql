-- KRITISCHER FIX: Security Definer Funktion muss RLS umgehen können
-- Gebe der Funktion die notwendigen Rechte

-- Alternative: Erstelle eine Funktion, die explizit als Service Role läuft
CREATE OR REPLACE FUNCTION public.save_daily_metrics_final(
  p_user_id UUID,
  p_metric_date DATE,
  p_data JSONB
)
...
DECLARE
  result_data JSONB;
BEGIN
  -- Wichtig: Diese Funktion läuft als SECURITY DEFINER (= service role)
  -- RLS wird automatisch umgangen für service role
