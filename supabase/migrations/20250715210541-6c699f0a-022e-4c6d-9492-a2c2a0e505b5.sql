-- KOMPLETTE LÖSUNG: Entferne die alte Funktion und erstelle eine neue, robuste
DROP FUNCTION IF EXISTS public.save_daily_metrics(uuid, date, jsonb);

-- Erstelle eine WIRKLICH robuste Funktion, die ohne auth.uid() Abhängigkeit funktioniert
...
DECLARE
  result_data JSONB;
BEGIN
  -- Einfache UPSERT Operation mit dem korrekten unique constraint
